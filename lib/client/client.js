...
class Client {
  ...
  listenForCommands(messageTypes) {
    this.slackController.hears(".*", messageTypes, (bot, message) => {
      ...
      let omitPrefix = message.type === 'direct_message' || message.type === 'direct_mention';

      this.slackController.log(message.type);
      this.slackController.log(message.text);

      let command = this.parseCommand(message.text, omitPrefix);
      if (command) {
        command.run(message, !omitPrefix);
      }
    });
  }

  // Parse a given message text for a command
  parseCommand(messageText, omitPrefix) {
    ...
    Array.from(this.commands.values()).forEach((testCommand) => {
      let commandFormat = (omitPrefix ? '' : this.commandPrefix) + testCommand.name;
